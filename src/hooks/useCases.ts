    ...
    queryFn: async () => {
      ...
      const cases: Case[] = response.data.map((row: any) => {
        ...
        return {
          ...
          ageMins,
          sla: row.sla_state === 'breached' ? 'breached' : row.sla_state === 'at_risk' ? 'at-risk' : 'ok',
      ...
      return {
        ...
        kpis: {
          ...
          agentsAvailable: Math.max(0, cases.filter(c => c.agentId).length - cases.filter(c => c.status === 'in_progress').length),
          oldestWaitingMins: Math.max(0, ...cases.filter(c => c.status === 'unassigned').map(c => c.ageMins)),
        },
        priorityMix: pMix,
        statusMix: sMix,
      };
