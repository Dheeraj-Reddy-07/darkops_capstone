...
async function runTests() {
  ...
  console.log(
    "No token /auth/me ->",
    unauth.status === 401 ? "PASS (401)" : `FAIL (${unauth.status})`,
  );
  ...
  console.log(
    "Agent /fraud ->",
    agentFraud.status === 403 ? "PASS (403)" : `FAIL (${agentFraud.status})`,
  );

  const custStores = await request("/stores", custToken);
  console.log(
    "Customer /stores ->",
    custStores.status === 403 ? "PASS (403)" : `FAIL (${custStores.status})`,
  );
  ...
  console.log(
    "Agent /cases ->",
    agentCases.status === 200 && Array.isArray(agentCases.data.data)
      ? `PASS (${agentCases.data.data.length} cases)`
      : `FAIL (${agentCases.status}) ${JSON.stringify(agentCases.data)}`,
  );
  ...
  console.log(
    "Fraud /fraud ->",
    fraudCases.status === 200
      ? "PASS"
      : `FAIL (${fraudCases.status}) ${JSON.stringify(fraudCases.data)}`,
  );
  ...
  console.log(
    "Exec /stores ->",
    execStores.status === 200
      ? "PASS"
      : `FAIL (${execStores.status}) ${JSON.stringify(execStores.data)}`,
  );
  ...
  console.log(
    ...
    execInsights.status === 200 ? "PASS" : `FAIL (${execInsights.status})`,
  );
