  ...
  try {
    ...
    switch (nlpAnalysis.type) {
      ...
      default:
        ...
        return await escalateToSupport(complaintId, complaint, nlpAnalysis, 'Unknown complaint type');
    }
...
async function processRefund(
  ...
  try {
    ...
    if (validation.can_auto_approve) {
      ...
      // Escalate to support
      return await escalateToSupport(complaintId, complaint, nlpAnalysis, validation.reason || 'Refund validation failed');
    }
  } catch (error) {
...
async function processReorder(
  ...
  try {
    ...
    if (validation.can_auto_approve) {
      ...
      // Escalate to support
      return await escalateToSupport(complaintId, complaint, nlpAnalysis, validation.reason || 'Reorder validation failed');
    }
  } catch (error) {
    return await logFailedAutomation(
      complaintId,
...
async function escalateToSupport(
  ...
  try {
    ...
    if (complaint.category === 'late_delivery' || complaint.category === 'quality_issue') queue = 'operational';
