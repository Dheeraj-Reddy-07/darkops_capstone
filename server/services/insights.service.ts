  ...
  private async buildContext(): Promise<NetworkContext> {
    ...
    const [
      ...
      supabase
        .from("stores")
        .select(
          "id, name, city, pulse_scores(score), store_metrics_snapshots(sla_pct, refund_rate_pct)",
        )
        .order("id")
        .limit(50),
      supabase
    ...
    return [
      {
        ...
        description: `There are ${criticalStores} stores with a PulseScore below 60. Immediate intervention recommended for: ${worstStores
          .slice(0, 3)
          .map((s) => s.name)
          .join(", ")}.`,
...
Question: ${question}`;
  ...
  private deterministicAnswer(question: string, ctx: NetworkContext): InsightAnswer {
    ...
    const worstStoreRefs = worstStores.map((s) => ({
      id: s.id,
      note: `${s.name} · Pulse ${s.pulse} · SLA ${s.sla}%`,
    }));
    ...
    if (q.includes("sla") || q.includes("breach")) {
      return {
        question,
        answer: `There are currently ${slaBreached} SLA breaches across the network. The worst-performing stores by SLA are ${worstStores
          .slice(0, 3)
          .map((s) => `${s.name} (${s.sla}%)`)
          .join(", ")}.`,
        metrics: [
          {
            ...
            tone: slaBreached > 20 ? "crit" : "warn",
          },
          { label: "P1 Cases", value: String(p1Cases), tone: p1Cases > 10 ? "crit" : "warn" },
          {
            label: "Network Pulse",
            value: `${avgPulse}/100`,
            tone: avgPulse < 60 ? "crit" : avgPulse < 80 ? "warn" : "ok",
          },
        ],
        ...
        metrics: [
          {
            ...
            tone: criticalStores > 10 ? "crit" : "warn",
          },
          {
            label: "Network Avg Pulse",
            value: `${avgPulse}/100`,
            tone: avgPulse < 70 ? "warn" : "ok",
          },
    ...
    if (q.includes("refund") || q.includes("fraud") || q.includes("risk")) {
      ...
      return {
        question,
        answer: `${highRefundStores.length} stores have refund rates above 5%. The top complaint categories driving refunds are: ${topCategories
          .slice(0, 3)
          .map((c) => c.category)
          .join(", ")}.`,
        metrics: [
          {
            label: "High Refund Stores",
            value: String(highRefundStores.length),
            tone: highRefundStores.length > 5 ? "crit" : "warn",
          },
          { label: "Active Cases", value: String(activeCases), tone: "neutral" },
        ],
        stores: highRefundStores
          .slice(0, 3)
          .map((s) => ({ id: s.id, note: `${s.name} · Refunds ${s.refundRate}%` })),
        causes: [
        ...
        action:
          "Route high-value claims through risk review before auto-approval and clear backlog.",
    ...
    return {
      ...
      metrics: [
        ...
        },
        {
          ...
          tone: activeCases > 100 ? "crit" : "warn",
        },
        {
          label: "SLA Breaches",
          value: String(slaBreached),
