...
function requireSupportRole(auth: any) {
  if (!['CUSTOMER_SUPPORT', 'PLATFORM_ADMIN'].includes(auth.user.role)) {
    throw new HTTPError(403, 'FORBIDDEN', 'Support workspace requires CUSTOMER_SUPPORT or PLATFORM_ADMIN role');
  }
}
...
export const updateTicketStatus = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const VALID_STATUSES = ['open', 'in_progress', 'awaiting_customer', 'escalated', 'resolved', 'closed'];
  ...
  try {
    ...
    if (assign_to_self || !agent_id) {
      ...
      if (auth.user.role !== 'PLATFORM_ADMIN') {
        throw new HTTPError(403, 'FORBIDDEN', 'Only administrators can reassign tickets to other agents');
      }
    ...
    await adminClient.from('ticket_activity').insert({
      ...
      actor_id: auth.user.id,
      event_type: 'assigned',
      payload: { from: oldAssignee, to: targetAgentId, to_name: agentProfile?.full_name || 'Unknown' },
    });
...
export const resolveTicket = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!resolution_note || String(resolution_note).trim().length < 5) {
      throw new HTTPError(400, 'INVALID_INPUT', 'Resolution note is required (minimum 5 characters)');
    }
...
export const addTicketNote = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { data, error } = await adminClient.from('ticket_activity').insert({
      ticket_id: id,
      actor_id: auth.user.id,
...
export const getAttachmentUploadUrl = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (signedErr) throw new HTTPError(500, 'STORAGE_ERROR', `Could not generate upload URL: ${signedErr.message}`);

    res.json({ signed_url: signedData.signedUrl, storage_path: storagePath, token: signedData.token });
  } catch (error) {
    next(error);
  }
};
...
export const createSupportTicket = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!ticketTitle && complaint_id) {
      const { data: complaint } = await adminClient.from('complaints').select('summary').eq('id', complaint_id).single();
