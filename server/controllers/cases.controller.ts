...
export const getCaseById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // IDOR Protection: Check if user has access to this case
    if (auth) {
      const userRole = auth.user.role;
      const userId = auth.user.id;
      const assignedAgentId = (data as any).assigned_agent_id;

      // PLATFORM_ADMIN and EXECUTIVE can access all cases
      if (userRole === "PLATFORM_ADMIN" || userRole === "EXECUTIVE") {
        // Allow access
      }
      // OPERATIONS and CUSTOMER_SUPPORT can access if assigned to them or unassigned
      else if (
        (userRole === "OPERATIONS" || userRole === "CUSTOMER_SUPPORT") &&
        ...
        throw new HTTPError(403, "FORBIDDEN", "You do not have permission to access this case.");
      }
    }
